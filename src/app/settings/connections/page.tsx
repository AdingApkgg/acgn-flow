...
export default function ConnectionsPage() {
  ...
  const { data: linkedAccounts, isLoading } = trpc.user.getLinkedAccounts.useQuery(
    undefined,
    { enabled: !!session }
  );
  ...
  useEffect(() => {
    if (status === "unauthenticated") {
      router.push("/login?callbackUrl=/settings/connections");
    }
  }, [status, router]);
  ...
  const handleLink = async (providerId: string) => {
    ...
    try {
      await signIn(providerId, { callbackUrl: "/settings/connections?linked=1" });
    } catch {
      toast.error("绑定失败");
  ...
  const isLinked = (providerId: string) => 
    linkedAccounts?.some((account) => account.provider === providerId);
  ...
  return (
    <div className="space-y-8">
      {/* 页面标题 */}
      <div>
        <h2 className="text-xl font-semibold">账号绑定</h2>
          ...
          return (
            ...
            >
              <div className="flex items-center gap-4">
                <div className={`p-2.5 rounded-lg ${provider.iconBg}`}>
                  <Icon className={`h-5 w-5 ${provider.iconClass}`} />
                </div>
                <div>
      ...
      <AlertDialog open={!!unlinkingProvider} onOpenChange={() => setUnlinkingProvider(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              解绑后将无法使用该第三方账号登录。如果你没有设置密码且这是唯一的登录方式，解绑将会失败。
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>取消</AlertDialogCancel>
            <AlertDialogAction
              onClick={() => unlinkingProvider && unlinkMutation.mutate({ provider: unlinkingProvider })}
